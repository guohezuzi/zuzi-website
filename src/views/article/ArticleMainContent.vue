...
export default {
  ...
  created() {
    ...
    // marked.setCallback(this.scrollHash())
    if (!articleContent) {
      const url =
        "/api/articles/articleContent.json?articleId=" + this.article.articleId;
  ...
  watch: {
    article: function() {
      if (this.article.articleContent != null) {
        this.scrollToHash();
      }
    }
  },
  methods: {
    scrollToHash() {
      ...
      if (id.length) {
        let ele = document.getElementById(id.substring(1).toLowerCase());
        ele.scrollIntoView();
      }
    }
  }
